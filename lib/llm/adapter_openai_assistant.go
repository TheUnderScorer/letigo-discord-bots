...
import (
	...
	"lib/util/arrayutil"
	"strings"
	openaiutil "wojciech-bot/openai"
)
...
func (o *OpenAIAssistantAdapter) Chat(ctx context.Context, chat *Chat) (*ChatMessage, *ChatReplyMetadata, error) {
	...
	if err != nil {
		return nil, nil, errors2.Wrap(err, "failed to count tokens")
	}
	...
	if resolvedThreadId, ok := chat.Metadata[threadIdMetadataKey]; ok {
		...
		if err != nil {
			...
			if err != nil {
				return nil, nil, errors2.Wrap(err, "failed to list thread message ids")
			}
			...
			for _, chatMessage := range userMessages {
				if chatMessage.ID == "" || !arrayutil.Includes(threadMessageIds, chatMessage.ID) {
					...
					if err != nil {
						return nil, nil, errors2.Wrap(err, "failed to send message to thread")
					}
				}
			}
		}
		...
		if err != nil {
			return nil, nil, errors2.Wrap(err, "failed to create thread")
		}
	...
	if err != nil {
		return nil, nil, errors2.Wrap(err, "failed to parse schema")
	}
	...
	for stream.Next() {
		err = stream.Err()
		if err != nil {
			return nil, nil, errors2.Wrap(err, "stream run error")
		}
		v := stream.Current()
			...
			if len(v.Data.Content) > 0 {
				...
				for _, content := range v.Data.Content {
					...
					if refusal.Refusal != "" {
						...
						if lastMessageOk {
							prompt = lastMessage.Contents
						}

						return nil, nil, NewRefusedToReplyError(refusal.Refusal, prompt)
					}
					...
					if text.Text.Value != "" {
						...
						if err != nil {
							...
							return nil, nil, errors2.Wrap(err, "failed to unmarshal thread response")
						}
...
func (o *OpenAIAssistantAdapter) handleAttachments(ctx context.Context, message *ChatMessage) ([]string, error) {
	...
	if len(message.Files) > 0 {
		for _, file := range message.Files {
			...
			if err != nil {
				return nil, errors2.Wrap(err, "failed to upload file")
			}
